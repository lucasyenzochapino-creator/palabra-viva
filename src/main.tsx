...
type BibleBook = { code?: string; bookName: string; testament: Testament | string; chapters: { chapter: number; verses: { verse: number; text: string }[] }[] };
...
const curated: Verse[] = [
  { id:'sal-23-1', book:'Salmos', chapter:23, verse:1, text:'Jehová es mi pastor; nada me faltará.', testament:'Antiguo Testamento', theme:['paz','cuidado'] },
  { id:'sal-27-1', book:'Salmos', chapter:27, verse:1, text:'Jehová es mi luz y mi salvación: ¿de quién temeré?', testament:'Antiguo Testamento', theme:['miedo','valentia'] },
  { id:'sal-34-18', book:'Salmos', chapter:34, verse:18, text:'Cercano está Jehová á los quebrantados de corazón.', testament:'Antiguo Testamento', theme:['tristeza','duelo'] },
  { id:'sal-46-1', book:'Salmos', chapter:46, verse:1, text:'Dios es nuestro amparo y fortaleza, nuestro pronto auxilio en las tribulaciones.', testament:'Antiguo Testamento', theme:['proteccion','miedo'] },
  { id:'prov-3-5', book:'Proverbios', chapter:3, verse:5, text:'Fíate de Jehová de todo tu corazón, y no estribes en tu prudencia.', testament:'Antiguo Testamento', theme:['decision','direccion'] },
  { id:'isa-41-10', book:'Isaías', chapter:41, verse:10, text:'No temas, porque yo soy contigo; no desmayes, porque yo soy tu Dios.', testament:'Antiguo Testamento', theme:['soledad','miedo'] },
  ...
  { id:'juan-3-16', book:'Juan', chapter:3, verse:16, text:'Porque de tal manera amó Dios al mundo, que ha dado á su Hijo unigénito, para que todo aquel que en él cree, no se pierda, mas tenga vida eterna.', testament:'Nuevo Testamento', theme:['jesus','evangelio','empezar'] },
  { id:'juan-14-6', book:'Juan', chapter:14, verse:6, text:'Jesús le dice: Yo soy el camino, y la verdad, y la vida: nadie viene al Padre, sino por mí.', testament:'Nuevo Testamento', theme:['jesus','camino','empezar'] },
  { id:'juan-14-27', book:'Juan', chapter:14, verse:27, text:'La paz os dejo, mi paz os doy: no como el mundo la da, yo os la doy.', testament:'Nuevo Testamento', theme:['paz','ansiedad'] },
  { id:'rom-8-28', book:'Romanos', chapter:8, verse:28, text:'Y sabemos que á los que á Dios aman, todas las cosas les ayudan á bien.', testament:'Nuevo Testamento', theme:['esperanza','proceso'] },
  ...
  { id:'1tim-4-12', book:'1 Timoteo', chapter:4, verse:12, text:'Ninguno tenga en poco tu juventud; pero sé ejemplo de los fieles.', testament:'Nuevo Testamento', theme:['jovenes','identidad'] },
...
function App() {
  ...
  React.useEffect(() => {
    ...
    fetch('/bible.rv1909.json', { cache: 'no-store' })
      ...
      .then(data => {
        if (cancelled) return;
        const books: BibleBook[] = Array.isArray(data.books)
          ? data.books.filter((b: BibleBook) => b.bookName && Array.isArray(b.chapters)).sort((a: BibleBook, b: BibleBook) => canonicalIndex(a.bookName) - canonicalIndex(b.bookName))
          : [];
        setBible(books);
        if (books.length > 0) {
          const john = books.find(b => normalize(b.bookName) === 'juan') || books.find(b => b.testament === 'Nuevo Testamento') || books[0];
          setBookName(john.bookName);
          setTestament(john.testament === 'Antiguo Testamento' ? 'Antiguo Testamento' : 'Nuevo Testamento');
  ...
  const books = React.useMemo(() => {
    const sourceBooks = bible.length ? bible : Array.from(new Set(curated.map(v => v.book))).map(book => ({ bookName: book, testament: curated.find(v => v.book === book)?.testament || 'Nuevo Testamento', chapters: [] }));
    return sourceBooks
      .filter(b => b.testament === testament)
      .sort((a, b) => canonicalIndex(a.bookName) - canonicalIndex(b.bookName))
      .map(b => b.bookName);
  }, [bible, testament]);
  ...
  const bibleSearchResults = allVerses
    .filter(v => v.testament === testament)
    .filter(v => {
      const q = normalize(bibleQuery);
      return q && (normalize(v.text).includes(q) || normalize(v.book).includes(q) || `${normalize(v.book)} ${v.chapter}`.includes(q));
    })
    .slice(0, 80);
  ...
  const changeTestament = (next: Testament) => {
    setTestament(next);
    setBibleQuery('');
    const first = (bible.length ? bible : Array.from(new Set(curated.map(v => v.book))).map(book => ({ bookName: book, testament: curated.find(v => v.book === book)?.testament || 'Nuevo Testamento', chapters: [] })))
      .filter(b => b.testament === next)
      .sort((a, b) => canonicalIndex(a.bookName) - canonicalIndex(b.bookName))[0];
    if (first) {
      setBookName(first.bookName);
      const firstChapter = bible.find(b => b.bookName === first.bookName)?.chapters[0]?.chapter || curated.find(v => v.book === first.bookName)?.chapter || 1;
  ...
  const openReference = (ref: string) => {
    ...
    const allBooks = bible.length ? bible.map(b => b.bookName) : Array.from(new Set(curated.map(v => v.book)));
    const found = allBooks.find(b => normalize(b) === wantedBook || normalize(b).includes(wantedBook) || wantedBook.includes(normalize(b)));
    if (found) {
      const t = bible.find(b => b.bookName === found)?.testament || curated.find(v => v.book === found)?.testament || 'Nuevo Testamento';
      setTestament(t === 'Antiguo Testamento' ? 'Antiguo Testamento' : 'Nuevo Testamento');
      setBookName(found);
  ...
  return <main className="app">
    ...
    {tab === 'hoy' && <div className="stack">
      <Card className="visualControls"><div><p className="ref">Pantalla</p><h3>Lectura cómoda</h3></div><div className="screenTools"><button className={theme === 'dark' ? 'tool active' : 'tool'} onClick={() => setTheme('dark')}>Oscuro</button><button className={theme === 'light' ? 'tool active' : 'tool'} onClick={() => setTheme('light')}>Claro</button><button className={theme === 'sepia' ? 'tool active' : 'tool'} onClick={() => setTheme('sepia')}>Sepia</button><button className="tool" onClick={() => setFont(Math.max(16, font - 1))}>A-</button><button className="tool" onClick={() => setFont(Math.min(24, font + 1))}>A+</button></div></Card>
      <section className="hero"><div className="pill"><Sparkles size={16}/> Palabra diaria</div><h2>Fe clara para el día real.</h2><p>Leé, guardá, compartí y seguí tu camino con más paz.</p></section>
      <Card className="today"><div className="row between"><p className="ref">{todayVerse.book} {todayVerse.chapter}:{todayVerse.verse}</p><Flame size={22}/></div><p className="verse big">“{todayVerse.text}”</p><p className="soft">Una palabra breve para acercarte a Dios sin complicarte.</p><p><strong>Oración breve:</strong> Señor, hablame con claridad y ayudame a vivir este día con sabiduría.</p><div className="row wrap"><Button onClick={() => toggleSave(todayVerse.id || vid(todayVerse))}>Guardar</Button><Button variant="ghost" onClick={() => navigator.share ? navigator.share({title:'Palabra para hoy', text:`${todayVerse.book} ${todayVerse.chapter}:${todayVerse.verse}\n\n${todayVerse.text}`}) : navigator.clipboard.writeText(todayVerse.text)}><Share2 size={16}/> Compartir palabra</Button></div></Card>
      <Card className="introCard"><div><p className="ref">Nuevo camino</p><h3>¿No sabés por dónde empezar con Jesús?</h3><p className="soft">Una guía simple para arrancar sin presión y con lecturas claras.</p></div><Button variant="ghost" onClick={() => setTab('empezar')}><Cross size={16}/> Ver guía</Button></Card>
      <div className="grid2"><Card className="mini" onClick={() => setTab('biblia')}><BookOpen/> Leer Biblia</Card><Card className="mini" onClick={() => setTab('sentir')}><Heart/> ¿Cómo venís?</Card><Card className="mini" onClick={() => setTab('buscar')}><Search/> Buscar palabra</Card><Card className="mini" onClick={() => setTab('planes')}><CalendarDays/> Planes</Card></div>
    </div>}
