...
function VerseCard({verse,onSave,saved,onNote}:{verse:Verse;onSave:()=>void;saved:boolean;onNote?:()=>void}){
  ...
  const shareImage=()=>{const ref=`${verse.book} ${verse.chapter}:${verse.verse}`;const W=(window as any);if(W.PVShareImage?.share)W.PVShareImage.share({text:verse.text,ref});else if(W.PVShareVerse?.preview)W.PVShareVerse.preview({book:verse.book,chapter:verse.chapter,verse:verse.verse,text:verse.text});else shareText();};
